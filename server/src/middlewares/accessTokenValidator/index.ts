...
export const accessTokenValidator = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!accessToken) return throwError(Status['not-auth'], res, ErrorMessages.nonAuthorized)
  jwt.verify(accessToken, ENV?.JWT_ACCESS_TOKEN_SECRET, (error: any, decoded: any) => {
    if (error) return throwError(Status['token-expired'], res, ErrorMessages.haventAccessRights)
