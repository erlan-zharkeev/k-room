...
export const validateFileMetaData = (
  ...
  if (filedata.metadata.size > maxBytes) {
    throwHTTPError(StatusEnum.BadRequest, res, getLocalizedText(VALIDATE_MEDIA_FILE_I18N.fileIsTooLarge, language))
  }

  if (filedata.metadata.kind !== supportedKindMediaType) {
    throwHTTPError(StatusEnum.BadRequest, res, getLocalizedText(VALIDATE_MEDIA_FILE_I18N.extNotSupported, language))
  }
  return true
}
