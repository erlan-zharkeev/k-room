...
interface Item extends Pick<IUserSchema, '_id' | 'avatarPath' | 'username' | 'email' | 'role' | 'online' | 'confirmed'> { }
...
export const UsersTable: React.FC<{ originData: Item[] }> = ({ originData }) => {
  ...
  const updateUserDataHandler = async (userData: UserDataToSave) => {
    try {
      ...
      return doRequest('post', AdminEndpoints.UPDATE_USER_DATA, { userData });
    } catch (e: unknown) {
  ...
  const save = async (key: string) => {
    try {
      ...
      if (index > -1) {
        const response = await updateUserDataHandler({ id: key, role: row.role, username: row.username, email: row.email, confirmed: row.confirmed });
  ...
  const deleteUser = async (id: string) => {
    ...
    try {
      const response = await doRequest('post', AdminEndpoints.DELETE_USER, { deleteUserId: id });
          ...
          <div className='users-table__operations-wrapper'>
            {isLoading ? (
              <div className='users-table__operations'>
                <UIIcon name="loader" color="accent" size="small" />
              </div>
            ) : (
              <div className='users-table__operations'>
                <UIButton text='Edit' disabled={editingKey !== ''} onClick={() => edit(record)} color="accent" />
                <Popconfirm title="Sure to delete user?" onConfirm={() => deleteUser(record._id)}>
                  <UIButton iconName='trash' color='error' disabled={editingKey !== ''} />
                </Popconfirm>
              </div>
            )}
          </div>
