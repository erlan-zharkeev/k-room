...
export const SelectUsers = ({
  ...
  setMembers: React.Dispatch<React.SetStateAction<UsersToSelect>>
}) => {
  ...
  }, [contacts, selectedUsers]) // Обновляем список при изменении контактов или выбранных пользователей
    ...
    <div className="select-users">
      ...
      <div className="select-users__choose-list">
        {contactsToSelect.map((contact) => (
          <div
            className="select-users__user"
            key={contact.id}
            onClick={() => onClickHandler(contact)}
          >
            <UIAvatar shape="square" src={contact.avatarPath} />
            <span className="paragraph-text paragraph-text--secondary select-users__name">
              {contact.username}
            </span>
          </div>
        ))}
