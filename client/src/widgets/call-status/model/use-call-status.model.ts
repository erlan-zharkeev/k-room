...
import { isRoomCallDeclinedByUser, useRoomCall } from 'src/entities/room-call'
...
export const useCallStatus = () => {
  ...
  const availableRoomCalls = computed(() =>
    visibleRoomCalls.value.filter((roomCall) => !isRoomCallDeclinedByUser(roomCall, user.value.id))
  )
  ...
  const incomingRoomCall = computed(() =>
    availableRoomCalls.value.find((roomCall) => {
