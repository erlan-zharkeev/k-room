...
import { AppAvatar, AppIcon, AppButton, AppInput } from 'src/shared/ui'
import { useApi } from 'src/shared/api'

interface Item
  extends Pick<IUserSchema, 'id' | 'avatarPath' | 'username' | 'email' | 'role' | 'online' | 'confirmed'> {}
...
const EditableCell: React.FC<EditableCellProps> = ({
  ...
  if (inputType === 'checkbox') {
    ...
    inputNode = <AppInput name="pointer" value="" />
  }
  ...
  const columns = [
    {
      ...
      render: (_: unknown, record: Item) => (
        <div className="avatar-path-column">
          <AppAvatar src={record.avatarPath} online={record.online} />
        </div>
      )
    },
    {
          ...
          <div className="users-table__operations-wrapper">
            {isLoading ? (
              <div className="users-table__operations">
                <AppIcon name="loader" color="accent-color" size="small" />
              </div>
            ) : (
              <div className="users-table__operations">
                <AppButton
                  ...
                  borderless
                />
                <Popconfirm title="Sure to delete user?" onConfirm={() => deleteUser(record.id)}>
                  <AppButton prefixIconName="trash" color="error-color" disabled={editingKey !== ''} borderless />
                </Popconfirm>
              </div>
            )}
          </div>
