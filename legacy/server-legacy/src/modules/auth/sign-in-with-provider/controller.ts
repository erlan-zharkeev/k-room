...
import { SignInWithProviderPayload, ISignInWithProviderResponse, REQ_STATUS } from 'common'
...
export const signInWithProviderController = async (req: AppRequest, res: AppResponse<ISignInWithProviderResponse>) => {
  const { language } = req
  const basicError = localizedText(SIGN_IN_WITH_PROVIDER_I18N.failed, language)

  try {
    const data: SignInWithProviderPayload = req.body
